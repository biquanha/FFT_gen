...
#define hard_POINT     31      // 定点数精度
#define TEST_ITERATIONS  5      // 测试迭代次数
...
int hardware_fft_model(Complex *input, Complex *output, int length) {
    ...
    int64_t re_fixed ;
    int64_t im_fixed ;
    volatile uint64_t *output_base = (volatile uint64_t*)0x43c10000;
    volatile uint64_t *input_base = (volatile uint64_t*)0x43C10000;
    for (int i = 0; i < length; i++) {
        // 转换为定点数格式
  	  re_fixed = (int64_t)(input[i].re * (1 << hard_POINT));
  	   im_fixed = (int64_t)(input[i].im * (1 << hard_POINT));

        int64_t wr_data =(re_fixed <<32) | im_fixed;
        *input_base = wr_data;
    }
    i=0;
      for (int i = 0; i < length; i++) {
          // 转换为定点数格式
          int64_t rd_data= *input_base ;
          re_fixed = rd_data >>32 ;
          im_fixed = (rd_data  & 0x0000FFFF);

          output[i].re = (double)re_fixed / (1 << hard_POINT);
          output[i].im = (double)im_fixed / (1 << hard_POINT);
      }

    printf(" xilinx  FFT run end\n");
...
int custom_fft_model(Complex *input, Complex *output, int length) {
    ...
    int64_t re_fixed;
    int64_t im_fixed ;
    volatile uint64_t *output_base = (volatile uint64_t*)0x60000000;
    volatile uint64_t *input_base = (volatile uint64_t*)0x60000000;
      for (int i = 0; i < length; i++) {
          // 转换为定点数格式
          int64_t re_fixed = (int64_t)(input[i].re * (1 << BINARY_POINT));
          int64_t im_fixed = (int64_t)(input[i].im * (1 << BINARY_POINT));
...
TestResult compare_fft_models(const char *name1, fft_func_t func1,
    ...
    printf("output1 address: %p\n", (void*)output1);
      printf("output2 address: %p\n", (void*)output2);
   ...
   // clock_t start1 = clock();
    ...
    XTime_GetTime(&tEnd);
   // clock_t end1 = clock();
    result.sw_time_ms = ((tEnd-tCur)*1000000)/(COUNTS_PER_SECOND);
  ...
  //  clock_t start2 = clock();
    ...
    //clock_t end2 = clock();
    XTime_GetTime(&tEnd);
    result.hw_time_ms = ((tEnd-tCur)*1000000)/(COUNTS_PER_SECOND);
